...
function closeSplash(){ if(splash) splash.classList.add('hide'); }
...
if(navModal) navModal.addEventListener('click', e => { if(e.target === navModal) navModal.classList.remove('show'); });

document.querySelectorAll('[data-google]').forEach(btn => {
  btn.addEventListener('click', () => { window.location.href = CONFIG.googleMapsUrl || 'https://maps.app.goo.gl/2QyUj3udpuiFAStJ6'; });
});

document.querySelectorAll('[data-waze]').forEach(btn => {
  btn.addEventListener('click', () => { window.location.href = CONFIG.wazeUrl || 'https://waze.com/ul/hw284r2bty'; });
});
...
if(videoLayer) videoLayer.addEventListener('click', e => { if(e.target === videoLayer) closeVideoPlayer(); });
